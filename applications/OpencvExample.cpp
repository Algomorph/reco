...
void testStereo(const char*imgfilename1, const char*imgfilename2, double minshift, double maxshift)
	...
	if (img1 && img2 && imgLeftDisparity && imgRightDisparity && imgLeftOcclusions
			&& imgRightOcclusions)
			{
		//wrap all the input and output images in OpenCVImageAdapter, so that they can be
		//accessed by OpenVis3D
		LegacyOpenCVImageAdapter*ovaImg1 = new LegacyOpenCVImageAdapter(img1);
		LegacyOpenCVImageAdapter*ovaImg2 = new LegacyOpenCVImageAdapter(img2);
		LegacyOpenCVImageAdapter*ovaLeftDisp = new LegacyOpenCVImageAdapter(imgLeftDisparity);
		LegacyOpenCVImageAdapter*ovaRightDisp = new LegacyOpenCVImageAdapter(imgRightDisparity);
		LegacyOpenCVImageAdapter*ovaLeftOcc = new LegacyOpenCVImageAdapter(imgLeftOcclusions);
		LegacyOpenCVImageAdapter*ovaRightOcc = new LegacyOpenCVImageAdapter(imgRightOcclusions);
	...
	if (img2)
		cvReleaseImage(&img2);
	if (imgLeftDisparity)
		cvReleaseImage(&imgLeftDisparity);
	if (imgRightDisparity)
		cvReleaseImage(&imgRightDisparity);
	if (imgLeftOcclusions)
		cvReleaseImage(&imgLeftOcclusions);
	if (imgRightOcclusions)
		cvReleaseImage(&imgRightOcclusions);
}

void testOpticalFlow(const char*imgfilename1, const char*imgfilename2, double minshiftX,
		double maxshiftX, double minshiftY, double maxshiftY)
		{
	//read input images
	IplImage*img1 = cvLoadImage(imgfilename1);
	IplImage*img2 = cvLoadImage(imgfilename2);
	...
	if (img1 && img2 && imgU1 && imgV1 && imgO1 && imgU2 && imgV2 && imgO2)
		...
		LegacyOpenCVImageAdapter*ovaImg1 = new LegacyOpenCVImageAdapter(img1);
		LegacyOpenCVImageAdapter*ovaImg2 = new LegacyOpenCVImageAdapter(img2);
		LegacyOpenCVImageAdapter*ovaImgU1 = new LegacyOpenCVImageAdapter(imgU1);
		LegacyOpenCVImageAdapter*ovaImgV1 = new LegacyOpenCVImageAdapter(imgV1);
		LegacyOpenCVImageAdapter*ovaImgO1 = new LegacyOpenCVImageAdapter(imgO1);
		LegacyOpenCVImageAdapter*ovaImgU2 = new LegacyOpenCVImageAdapter(imgU2);
		LegacyOpenCVImageAdapter*ovaImgV2 = new LegacyOpenCVImageAdapter(imgV2);
		LegacyOpenCVImageAdapter*ovaImgO2 = new LegacyOpenCVImageAdapter(imgO2);
		...
		flowManager.doOpticalFlow(*ovaImg1, *ovaImg2, minshiftX, maxshiftX, minshiftY, maxshiftY,
				*ovaImgU1, *ovaImgV1, *ovaImgO1, *ovaImgU2, *ovaImgV2, *ovaImgO2);
	...
	if (img2)
		cvReleaseImage(&img2);
	if (imgU1)
		cvReleaseImage(&imgU1);
	if (imgV1)
		cvReleaseImage(&imgV1);
	if (imgO1)
		cvReleaseImage(&imgO1);
	if (imgU2)
		cvReleaseImage(&imgU2);
	if (imgV2)
		cvReleaseImage(&imgV2);
	if (imgO2)
		cvReleaseImage(&imgO2);
}
