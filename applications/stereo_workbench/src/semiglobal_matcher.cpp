...
static void compute_disparity_SGBM(const Mat& img1, const Mat& img2,
	...
	static const uchar bit_reverse_table[] =
			{
					...
					0, 2, 0, 1, 0,
					6, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1,
					0, 2, 0, 1, 0,
					5, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1,
					0, 2, 0, 1, 0,
					7, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1,
					0, 2, 0, 1, 0,
					5, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1,
					0, 2, 0, 1, 0,
					6, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1,
	...
	Size block_size;
	block_size.width = block_size.height = params.block_size > 0 ? params.block_size : 5;
	...
	int SW2 = block_size.width / 2, SH2 = block_size.height / 2;
						...
						if (y > 0){
							...
							for (x = D; x < width1 * D; x += D){
								...
								if (use_SIMD) {
									for (d = 0; d < D; d += 8)
											{
										__m128i hv = _mm_load_si128(
												(const __m128i *) (hsumAdd + x - D + d));
							...
							for (x = D; x < width1 * D; x += D){
								...
								for (d = 0; d < D; d++)
									hsumAdd[x + d] = (CostType) (hsumAdd[x - D + d] + pixAdd[d]
											- pixSub[d]);
							}
						}
					}
			...
			for (x = x1; x != x2; x += dx){
				...
				if (use_SIMD){
					...
					for (d = 0; d < D; d += 8)
						...
						L0 = _mm_min_epi16(L0,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p0 + d - 1)),
										_P1));
						L0 = _mm_min_epi16(L0,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p0 + d + 1)),
										_P1));
						...
						L1 = _mm_min_epi16(L1,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p1 + d - 1)),
										_P1));
						L1 = _mm_min_epi16(L1,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p1 + d + 1)),
										_P1));
						...
						L2 = _mm_min_epi16(L2,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p2 + d - 1)),
										_P1));
						L2 = _mm_min_epi16(L2,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p2 + d + 1)),
										_P1));
						...
						L3 = _mm_min_epi16(L3,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p3 + d - 1)),
										_P1));
						L3 = _mm_min_epi16(L3,
								_mm_adds_epi16(_mm_loadu_si128((const __m128i *) (Lr_p3 + d + 1)),
										_P1));
						...
						__m128i t0 = _mm_min_epi16(_mm_unpacklo_epi16(L0, L2),
								_mm_unpackhi_epi16(L0, L2));
						__m128i t1 = _mm_min_epi16(_mm_unpacklo_epi16(L1, L3),
								_mm_unpackhi_epi16(L1, L3));
				...
				#endif
				{
					...
					for (d = 0; d < D; d++){
						...
						L0 = Cpd
								+ std::min((int) Lr_p0[d],
										std::min(Lr_p0[d - 1] + P1,
												std::min(Lr_p0[d + 1] + P1, delta0))) - delta0;
						L1 = Cpd
								+ std::min((int) Lr_p1[d],
			...
			if (pass == npasses) {
				...
				for (x = width1 - 1; x >= 0; x--) {
					...
					if (npasses == 1) {
						...
						if (use_SIMD){
							...
							__m128i _minS = _mm_set1_epi16(MAX_COST), _bestDisp = _mm_set1_epi16(
									-1);
							__m128i _d8 = _mm_setr_epi16(0, 1, 2, 3, 4, 5, 6, 7), _8 =
									_mm_set1_epi16(8);
							...
							for (d = 0; d < D; d += 8)
								...
								L0 = _mm_min_epi16(L0,
										_mm_adds_epi16(
												_mm_loadu_si128((const __m128i *) (Lr_p0 + d - 1)),
												_P1));
								L0 = _mm_min_epi16(L0,
										_mm_adds_epi16(
												_mm_loadu_si128((const __m128i *) (Lr_p0 + d + 1)),
					...
					for (d = 0; d < D; d++){
						if (Sp[d] * (100 - uniquenessRatio) < minS * 100
								&& std::abs(bestDisp - d) > 1){
							break;
						}
					}
					if (d < D){
					...
					if (0 < d && d < D - 1){
						...
						d = d * DISP_SCALE
								+ ((Sp[d - 1] - Sp[d + 1]) * DISP_SCALE + denom2) / (denom2 * 2);
					}else{
						d *= DISP_SCALE;
					}
				...
				for (x = minX1; x < maxX1; x++){
					...
					if (0 <= _x && _x < width && disp2ptr[_x] >= minD
							&& std::abs(disp2ptr[_x] - _d) > disp12MaxDiff &&
							0 <= x_ && x_ < width && disp2ptr[x_] >= minD
							&& std::abs(disp2ptr[x_] - d_) > disp12MaxDiff)
...
void allocate_buffers(Mat& buffer, int width, int width1, int D, int num_ch, int SH2, int P2,
...
struct SGBM3WayMainLoop:
	...
	SGBM3WayMainLoop(Mat *_buffers, const Mat& _img1, const Mat& _img2, Mat* _dst_disp,
			const semiglobal_matcher_parameters& params, int _nstripes, int _stripe_overlap,
			pixel_cost_type cost_type = pixel_cost_type::BIRCHFIELD_TOMASI);

	void getRawMatchingCost(CostType* C, CostType* hsumBuf, CostType* pixDiff,
			int y, int src_start_idx) const;
	void operator ()(const Range& range) const;
};

SGBM3WayMainLoop::SGBM3WayMainLoop(Mat *_buffers, const Mat& _img1, const Mat& _img2,
		Mat* _dst_disp,
		const semiglobal_matcher_parameters& params, int _nstripes, int _stripe_overlap,
		pixel_cost_type cost_type) :
		buffers(_buffers), img1(&_img1), img2(&_img2), dst_disp(_dst_disp),
...
void allocate_buffers(Mat& buffer, int width, int width1, int D, int num_ch, int SH2, int P2,
	...
	horPassCostVolume = (CostType*) (pixDiff + width * 16 * num_ch);
...
void SGBM3WayMainLoop::getRawMatchingCost(CostType* C, // target cost-volume row
		CostType* hsumBuf, CostType* pixDiff, //buffers
		int y, int src_start_idx) const {
	int x, d;
			...
			if (y > src_start_idx){
				...
				for (x = D; x < width1 * D; x += D){
					...
					for (d = 0; d < D; d += 8){
						...
						v_store_aligned(C + x + d,
								v_load_aligned(C + x + d)
										+ (hv_reg - v_load_aligned(hsumSub + x + d)));
					}
#else
					for( d = 0; d < D; d++ )
					{
...
inline short min_pos(const v_int16x8& val, const v_int16x8& pos)
	...
	if (val_buf[0] < min_val) {
		min_val = val_buf[0];
		res_pos = pos_buf[0];
	}
	if (val_buf[1] < min_val) {
		min_val = val_buf[1];
		res_pos = pos_buf[1];
	}
	if (val_buf[2] < min_val) {
		min_val = val_buf[2];
		res_pos = pos_buf[2];
	}
	if (val_buf[3] < min_val) {
		min_val = val_buf[3];
		res_pos = pos_buf[3];
	}
	if (val_buf[4] < min_val) {
		min_val = val_buf[4];
		res_pos = pos_buf[4];
	}
	if (val_buf[5] < min_val) {
		min_val = val_buf[5];
		res_pos = pos_buf[5];
	}
	if (val_buf[6] < min_val) {
		min_val = val_buf[6];
		res_pos = pos_buf[6];
	}
	if (val_buf[7] < min_val) {
		min_val = val_buf[7];
		res_pos = pos_buf[7];
	}
	return res_pos;
...
inline void accumulateCostsLeftTop(CostType* leftBuf, CostType* leftBuf_prev, CostType* topBuf,
		CostType* costs,
	...
	for (int i = 0; i < D - 8; i += 8)
		...
		res = v_load_aligned(costs + i)
				+ (v_min(v_min(src_shifted_left, src_shifted_right),
						v_min(src1_leftBuf, leftMinCostP2_reg)) - leftMinCostP2_reg);
		...
		res = v_load_aligned(costs + i)
				+ (v_min(v_min(src_shifted_left, src_shifted_right),
						v_min(src1_topBuf, topMinCostP2_reg)) - topMinCostP2_reg);
...
inline void accumulateCostsRight(CostType* rightBuf, CostType* topBuf, CostType* leftBuf,
		CostType* costs,
	...
	for (int i = 0; i < D - 8; i += 8)
		...
		res = v_load_aligned(costs + i)
				+ (v_min(v_min(src_shifted_left, src_shifted_right),
						v_min(src1_rightBuf, rightMinCostP2_reg)) - rightMinCostP2_reg);
		...
		min_sum_pos_reg = min_sum_pos_reg
				+ ((min_sum_cost_reg == res) & (loop_idx - min_sum_pos_reg));
...
void SGBM3WayMainLoop::operator ()(const Range& range) const
	...
	short* disp2Buf;
	allocate_buffers(cur_buffer, width, width1, D, img1->channels(), SH2, P2,
			curCostVolumeLine, hsumBuf, pixDiff, horPassCostVolume,
			vertPassCostVolume, vertPassMin, rightPassBuf, disp2CostBuf, disp2Buf);

	// start real processing:
	for (int y = src_start_idx; y < src_end_idx; y++)
			{
		getRawMatchingCost(curCostVolumeLine, hsumBuf, pixDiff, y, src_start_idx);
		...
		for (int x = D; x < (1 + width1) * D; x += D)
			accumulateCostsLeftTop(horPassCostVolume + x, horPassCostVolume + x - D,
					vertPassCostVolume + x, C + x, prev_min, vertPassMin[x / D], D, P1, P2);
		...
		for (int x = width1 * D; x >= D; x -= D)
				{
			accumulateCostsRight(rightPassBuf, vertPassCostVolume + x, horPassCostVolume + x, C + x,
					prev_min, D, P1, P2, best_d, min_cost);
			...
			if (0 < d && d < D - 1)
				...
				//   then find minimum of the parabola.
				int denom2 = std::max(
						horPassCostVolume[x + d - 1] + horPassCostVolume[x + d + 1]
								- 2 * horPassCostVolume[x + d], 1);
				d = d * DISP_SCALE
						+ ((horPassCostVolume[x + d - 1] - horPassCostVolume[x + d + 1])
								* DISP_SCALE + denom2) / (denom2 * 2);
			}
			else
				d *= DISP_SCALE;
		...
		for (int x = minX1; x < maxX1; x++)
			...
			int _x = x - _d, x_ = x - d_;
			if (0 <= _x && _x < width && disp2Buf[_x] >= minD
					&& std::abs(disp2Buf[_x] - _d) > disp12MaxDiff &&
					0 <= x_ && x_ < width && disp2Buf[x_] >= minD
					&& std::abs(disp2Buf[x_] - d_) > disp12MaxDiff)
...
static void computeDisparity3WaySGBM(const Mat& img1, const Mat& img2,
	...
	parallel_for_(Range(0, nstripes),
			SGBM3WayMainLoop(buffers, img1, img2, dst_disp, params, nstripes, stripe_overlap,
					cost_type));
...
class semiglobal_matcher_implementation:
		public StereoSGBM
{
private:
	pixel_cost_type cost_type;
...
public:
	semiglobal_matcher_implementation() :
			cost_type(pixel_cost_type::BIRCHFIELD_TOMASI), params() {
	}
	...
	semiglobal_matcher_implementation(int _minDisparity, int _numDisparities, int _SADWindowSize,
			...
			int _mode) :
			cost_type(pixel_cost_type::BIRCHFIELD_TOMASI), params(_minDisparity, _numDisparities,
					_SADWindowSize,
	...
	void compute(InputArray leftarr, InputArray rightarr, OutputArray disparr) {
		...
		if (params.speckleWindowSize > 0) {
			filterSpeckles(disp, (params.minDisparity - 1) * StereoMatcher::DISP_SCALE,
...
Ptr<StereoSGBM> create_semiglobal_matcher(int minDisparity, int numDisparities, int block_size,
	...
	return Ptr<StereoSGBM>(
			new reco::stereo_workbench::semiglobal_matcher_implementation(minDisparity,
					numDisparities, block_size,
					P1, P2, disp12MaxDiff,
