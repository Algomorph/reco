...
#include <reco/utils/cpp_exception_util.h>.h>
...
void calculate_row_cost_L2( const Mat& img1, const Mat& img2, int y,
	...
	const int minX2 = std::max(minX1 - maxD, 0), maxX2 = std::min(maxX1 - minD, image_width);
	int D = maxD - minD;
                            ...
                            int tabOfs, int )
{
    ...
    //const double norm_factor = 60.0D;
	...
	if( channel_number == 1 ){
		for( x = 1; x < image_width-1; x++ ){
			...
			precomputed_buffer_img1[x] =
					lookup_table[(row_img1[x+1] - row_img1[x-1])*2 + //current pixel surround
			...
			precomputed_buffer_img2[image_width-1-x] =
					lookup_table[(row_img2[x+1] - row_img2[x-1])*2 +
		...
		for( x = 1; x < image_width-1; x++ ){
			...
			precomputed_buffer_img1[x] = //ch1
					lookup_table[(row_img1[x*3+3] - row_img1[x*3-3])*2 +
			...
			precomputed_buffer_img1[x+image_width] = //ch2
					lookup_table[(row_img1[x*3+4] - row_img1[x*3-2])*2 +
			...
			precomputed_buffer_img1[x+image_width*2] = //ch3
					lookup_table[(row_img1[x*3+5] - row_img1[x*3-1])*2 +
