...
#ifdef USE_STEREO_SGBM
				stereo_matcher(cv::StereoSGBM::create(0, 64, 3, 216, 864, -1, 48, 0, 0, 0, cv::StereoSGBM::MODE_SGBM)),
#elif USE_STEREO_BM
				stereo_matcher(cv::StereoBM::create(64,21)),
#else
#endif
#endif
...
//stereo_matcher(32, 8)
{
	...
	Eigen::Matrix<double,3,1> trans = T_nr_nl.translation();
	puts(trans);
}
...
bool stereo_processor::do_unit_of_work() {
	...
	if (array) {
		...
		calibu::Rectify(left_lut,copy_left.data,rect_left.data, copy_left.cols,copy_left.rows, copy_left.channels());
		calibu::Rectify(right_lut,copy_right.data,rect_right.data, copy_right.cols,copy_right.rows, copy_right.channels());
#else
