...
////////////////////////////////////////////////////////////////////////////////
Kinfu::Kinfu(int w, int h, bool useColour, roo::ImageIntrinsics K_depth, int volRes, float volRad, float mincostheta, float knear, float kfar, float trunc_dist_factor)
  // Input parameters
  : w_(w)
  , h_(h)
  ...
  // CUDA objects
  , reset_bb_(make_float3(-volRad_,-volRad_,knear_), make_float3(volRad_,volRad_,knear_+2*volRad_))  
  , dKinect_(w,h)
  , drgb_(w,h)
  , dKinectMeters_(w,h)
  , kin_d_(w,h)
  ...
  , s_cam_ (pangolin::ProjectionMatrixRDF_TopLeft(w_,h_,K_depth_.fu,K_depth_.fv,K_depth_.u0,K_depth_.v0,0.1,1000),
  ...
  , newFrame_(false)
{
  ...
  cout << "TSDF truncation distance: " << trunc_dist_factor_*length(vol_.VoxelSizeUnits()) * 100  << " centimetres" << endl;
...
////////////////////////////////////////////////////////////////////////////////
bool Kinfu::getViewonly() { return viewonly_; }
void Kinfu::setViewonly(bool viewonly) {viewonly_ = viewonly; };
...
bool Kinfu::spinOnce()
{
  ...
  if(pangolin::Pushed(reset_) || !isfinite(rmse_) )
  {
    ...
    const float trunc_dist = trunc_dist_factor_*length(vol_.VoxelSizeUnits());
    if(useColour_) {
      roo::SdfFuse(vol_, colorVol_, kin_d_[0], kin_n_[0], T_wl_.inverse().matrix3x4(), K_depth_, drgb_, (T_cd_ * T_wl_.inverse()).matrix3x4(), roo::ImageIntrinsics(rgb_fl_, drgb_), trunc_dist, max_w_, mincostheta_);
    }else{
      roo::SdfFuse(vol_, kin_d_[0], kin_n_[0], T_wl_.inverse().matrix3x4(), K_depth_, trunc_dist, max_w_, mincostheta_ );
    }
  }
  ...
  if(viewonly_) {
    ...
    if(work_vol.IsValid()) {
      if(showcolor_) {
        roo::RaycastSdf(ray_d_[0], ray_n_[0], ray_i_[0], work_vol, work_colorVol, T_vw.inverse().matrix3x4(), K_depth_, 0.1, 50, trunc_dist, true );
      }else{
        roo::RaycastSdf(ray_d_[0], ray_n_[0], ray_i_[0], work_vol, T_vw.inverse().matrix3x4(), K_depth_, 0.1, 50, trunc_dist, true );
      }
      ...
      if(keyframes_.size() > 0) {
        ...
        roo::TextureDepth<float4,uchar3,10>(ray_c_[0], kfs_, ray_d_[0], ray_n_[0], ray_i_[0], T_vw.inverse().matrix3x4(), K_depth_);
      }
    }
  }  
  ...
  else
  {
    ...
    const roo::BoundingBox roi(roo::BoundingBox(T_wl_.matrix3x4(), w_, h_, K_depth_, knear_,kfar_));
    roo::BoundedVolume<roo::SDF_t> work_vol = vol_.SubBoundingVolume( roi );
    if(work_vol.IsValid())
    {
      for(int l=0; l<maxLevels_; ++l) {
        if(its_[l] > 0) {
          ...
          if(showcolor_) {
            roo::RaycastSdf(ray_d_[l], ray_n_[l], ray_i_[l], work_vol, colorVol_, T_wl_.matrix3x4(), Kl, knear_,kfar_, trunc_dist, true );
          }else{
            roo::RaycastSdf(ray_d_[l], ray_n_[l], ray_i_[l], work_vol, T_wl_.matrix3x4(), Kl, knear_,kfar_, trunc_dist, true );
          }
        ...
        for(int l=maxLevels_-1; l >=0; --l)
        {
          ...
          for(int i=0; i<its_[l]; ++i ) {
            ...
            roo::LeastSquaresSystem<float,6> lss = roo::PoseRefinementProjectiveIcpPointPlane(
              kin_v_[l], ray_v_[l], ray_n_[l], mKT_lp, mT_pl, icp_c_, dScratch_, dDebug_.SubImage(0,0,w_>>l,h_>>l)
            );
    ...
    if(pose_refinement_ && fuse_) {
      if(tracking_good) {
        ...
        if(work_vol.IsValid()) {
          const float trunc_dist = trunc_dist_factor_*length(vol_.VoxelSizeUnits());
          if(useColour_) {
            roo::SdfFuse(work_vol, work_colorVol, kin_d_[0], kin_n_[0], T_wl_.inverse().matrix3x4(), K_depth_, drgb_, (T_cd_ * T_wl_.inverse()).matrix3x4(), roo::ImageIntrinsics(rgb_fl_, drgb_), trunc_dist, max_w_, mincostheta_ );
          }else{
//             roo::SdfFuse(work_vol, kin_d_[0], kin_n_[0], dKinectDisc_, T_wl_.inverse().matrix3x4(), KDepth_, trunc_dist, max_w_, mincostheta_ );
            roo::SdfFuse(work_vol, kin_d_[0], kin_n_[0], T_wl_.inverse().matrix3x4(), K_depth_, trunc_dist, max_w_, mincostheta_ );
  ...
  roo::BoundingBox bbox_work(T_wl_.matrix3x4(), w_, h_, K_depth_.fu, K_depth_.fv, K_depth_.u0, K_depth_.v0, knear_,kfar_);
