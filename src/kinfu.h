...
{  
public:
  Kinfu (int w, int h, bool useColour, double fx, double fy, double ox, double oy, int volRes, float volRad, float mincostheta=0.1, float knear = 0.4, float kfar = 4.0, float trunc_dist_factor = 2.0);
  ...
  bool addFrame(const cv::Mat &depth, const cv::Mat &rgb, const cv::Mat &depth_disc);
...
private:
  ...
  const int w_, h_;  
  const roo::ImageIntrinsics KDepth_;         // Depth sensor calibration parameters
  ...
  roo::Image<unsigned short, roo::TargetDevice, roo::Manage> dKinect_;      // Depth image
  roo::Image<uchar3, roo::TargetDevice, roo::Manage> drgb_;                 // RGB image
  roo::Image<unsigned char, roo::TargetDevice, roo::Manage> dKinectDisc_;           // Depth discontinuities
