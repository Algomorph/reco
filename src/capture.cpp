 ...
 public:
  ...
  void Run() {
    ...
    for (; !pangolin::ShouldQuit(); ++frame_number_) {
      ...
      if (delayed_logging_enabled_ && ((hal::Tic() - delayed_logging_start_) > logging_delay_))
      {
...
void ExtractImages(const std::string &log_file_name, const std::string &output_dir) {
  ...
  while (msg = reader.ReadMessage()) {
    if (msg->has_camera()) {
      ...
      for (int ii = 0; ii < cam_msg.image_size(); ++ii) {
        ...
        {
          filename = utl::fullfile(output_dir, "depth_" + convert.str() + ".pgm");
        ...
        {
          filename = utl::fullfile(output_dir, "rgb_" + convert.str() + ".png");
          cv::Mat imRGB (img_msg.height(), img_msg.width(), CV_8UC3, (void*)img_msg.data().data());
...
int main(int argc, char* argv[]) {
  ...
  {
    std::cout << "Output folder does not exists or is not a directory. (" << output_folder << ")" << std::endl;
    return -1;
  }
